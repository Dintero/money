...
const percentToMultiplier = (percent: Factor): Big =>
    Big(percent).add(100).div(100);
...
export class Money {
    ...
    addVat = (vatPercentage: Factor): Money => {
        return this.multiply(percentToMultiplier(vatPercentage)).setTag(
            "includesVat",
            true,
        );
    };
    ...
    removeVat = (vatPercentage: Factor): Money => {
        return this.divide(percentToMultiplier(vatPercentage)).setTag(
            "includesVat",
            false,
        );
    };
